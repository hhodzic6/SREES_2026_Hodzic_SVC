...
\title{\textbf{Dinamički konvertor za Static Var Compensator \\ (SVC Type I, SVC Type II) \\[0.3em] \large natID / dTwin plugin}}
\author{Haris Hodžić \\ Indeks: 19145}
\date{Seminarski rad iz predmeta SREES \\ Elektrotehnički fakultet, Univerzitet u Sarajevu \\[0.5em] \today}
...
\begin{itemize}
    \item ako čvor \textbf{nije} naveden u SVC XML konfiguraciji: standardni statički
        model (Slack: fiksni $e,f$; PV: regulacija napona sa mogućnošću prebacivanja u PQ
        kada se dostigne granica reaktivne snage generatora; PQ: fiksna $P,Q$ injekcija) -
        identičan format kao kod prošlogodišnjeg statičkog konvertora;
...
\texttt{ODEs} (samo za SVC čvorove), \texttt{NLEs} (za sve čvorove), \texttt{Limits}
(anti-windup i PV Q-limit switching) i \texttt{PostProc} (izlazne veličine za grafike:
napon, efektivna susceptansa/ugao, injektovana reaktivna snaga).

Primjer generisanih parametara i NLE jednačine za jedan SVC Type II čvor (bus 9,
...
cB9Free = true [type=bool out=true];
...
ODEs:
	if cB9Free:
		bSvc_9' = (Kr_9*(vref_9 - sqrt(e_9*e_9+f_9*f_9)) - bSvc_9)/Tr_9
	else:
		bSvc_9' = 0
	end
...
NLEs:
	(e_9*(G_9_4*e_4 - B_9_4*f_4) + f_9*(G_9_4*f_4 + B_9_4*e_4)) + ... - P_9_inj_spec = 0
	(f_9*(G_9_4*e_4 - B_9_4*f_4) - e_9*(G_9_4*f_4 + B_9_4*e_4)) + ...
		- bSvc_9*(e_9*e_9+f_9*f_9) + Q_9_load = 0
\end{lstlisting}

Vrijednosti $Y_{bus}$ matrice u generisanom fajlu (npr. $B_{1,1}=-17.3611$, $B_{4,4}=-39.3089$,
$B_{9,9}=-17.3382$) su provjerene i \textbf{poklapaju se tačno} sa vrijednostima iz
prošlogodišnjeg (Python) konvertora za isti \texttt{case9} slučaj, što potvrđuje ispravnost
implementacije izgradnje matrice admitansi. Kompletan generisani fajl (\texttt{case9\_svc.dmodl},
9837 bajtova) i pripadajući \texttt{case9\_svc.vmodl} nalaze se u
\texttt{src/TestSVCPlugin/res/testData/output/} u repozitoriju.

\section{XML parsiranje}

XML konfiguracija se parsira preko natID-ovog \texttt{xml::FileParser} (DOM stil
parsiranja, \texttt{getRootNode()}/\texttt{getChildNode()}/\texttt{getAttribValue()}), isti
obrazac kao u natID primjeru \texttt{04\_XmlIntro}.
...
\begin{enumerate}
    ...
    \item da anti-windup limiter ispravno ograničava stanje na zadatim granicama.
\end{enumerate}
...
Tools 2022, natID SDK) i pokrenut end-to-end za \texttt{case9} test slučaj (samostalan
konzolni test harness koji direktno poziva \texttt{Converter::run(...)}, van GUI-a).
Rezultat: generisan je ispravan \texttt{.dmodl} (9837 bajtova) i \texttt{.vmodl} fajl,
uz potvrdu da se vrijednosti $Y_{bus}$ matrice tačno poklapaju sa prošlogodišnjim
(nezavisno implementiranim) konvertorom za isti test slučaj (vidi Poglavlje
\ref{ch:implementacija}, Listing \ref{lst:dmodl-svc2}). Plugin (\texttt{svcConv.dll}) je
takođe uspješno učitan unutar prave dTwin aplikacije bez pada aplikacije.

\subsection{Tranzijentna simulacija u dTwin-u}

\todobox{Ovo poglavlje treba dopuniti nakon pokretanja same DAE simulacije (integracija
kroz vrijeme) unutar dTwin GUI-a - to zahtijeva vizuelnu interakciju sa aplikacijom koju
nije bilo moguće automatizovati u ovoj sesiji. Koraci: (1) otvoriti dTwin (plugin je već
prisutan u \texttt{C:\textbackslash Users\textbackslash User\textbackslash ba.natID\textbackslash dTwin\textbackslash plugins}); (2) pokrenuti SVC
konvertor plugin, konvertovati \texttt{case9.m} sa \texttt{svc\_config\_case9.xml} (ili
koristiti već generisani \texttt{res/testData/output/case9\_svc.dmodl}); (3) otvoriti taj
\texttt{.dmodl} u dTwin-u i pokrenuti vremensku simulaciju; (4) ovdje umetnuti grafike
napona/susceptanse/reaktivne snage iz \texttt{.vmodl} plotova (Figure
\ref{fig:rezultati-placeholder} je samo placeholder), i uporediti ponašanje SVC Type I i
SVC Type II čvora (očekuje se da Type II pokazuje nultu statičku grešku ali sa
vidljivijim oscilacijama, u skladu sa Milano Fig. 19.4).}

\begin{figure}[H]
\centering
\begin{tikzpicture}
\draw[dashed] (0,0) rectangle (10,5);
\node at (5,2.5) {\Large [ovdje umetnuti graf napona/Q iz dTwin simulacije]};
\end{tikzpicture}
\caption{Placeholder - zamijeniti stvarnim grafikom iz dTwin simulacije.}
\label{fig:rezultati-placeholder}
\end{figure}
...
\todobox{Dopuniti zaključak konkretnim zapažanjima nakon testiranja u dTwin-u (npr. da li
se ponašanje SVC Type I / Type II poklapa sa teorijskim očekivanjima, koliko dugo traje
konverzija za \texttt{case300}, itd).}
