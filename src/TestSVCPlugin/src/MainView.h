...
// Minimal standalone host application used to load and test svcConv.dll outside of the
// full dTwin app - mirrors natID.Examples/dTwin/Plugin/TestEQPlugin/src/MainView.h.
// PLUGIN_DIR (see testSVCPlugin.cmake) points at this build's own output folder, so the
// freshly built svcConv.dll/.dylib/.so is found automatically without any manual copying.
...
public:
    ...
    , _gl(5,2)
    {
        ...
        auto testDataPath = resPath / "testData/case9.m";
        if (fo::fileExists(testDataPath))
        {
            td::String strNote;
            strNote.format("Note: If the combo box above is not empty, click the Open button on the toolbar.\n\n"
                            "Then select this file as the MATPOWER input for the conversion:\n%s\n\n"
                            "...and this file as the SVC config XML:\n%s\n\n"
                            "The content of the test input file is given below (case9, 9-bus system)...",
                            testDataPath.c_str(), (resPath / "testData/svc_config_case9.xml").c_str());
